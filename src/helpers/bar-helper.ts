...
export const convertToBarTasks = (
  ...
  barTasks = barTasks.map((task) => {
    ...
    dependencies.forEach((dId: string) => {
      const dependence = barTasks.findIndex((value) => value.id === dId);
...
const handleTaskBySVGMouseEventForBar = (
  ...
  switch (action) {
    case 'progress':
      changedTask.progress = rtl
        ? progressByXRTL(svgX, selectedTask)
        : progressByX(svgX, selectedTask);
...
export const handleTaskBySVGMouseEvent = (
  ...
  if (selectedTask.type === 'milestone') {
